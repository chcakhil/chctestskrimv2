...
import { Lock, Search, MoreVertical, Edit, Phone, Video, ArrowLeft, MessageCircle, CheckCircle, XCircle, Play, Zap, Settings, Pin } from 'lucide-react';
...
function SwipeableChatRow({ chat, onClick }: { chat: any, onClick: () => void }) {
    const { width } = useWindowDimensions();
    const isMobile = width < 768;
    const [dragX, setDragX] = useState(0);

    const getStreakColor = (streak: number) => {
        if (streak >= 30) return 'text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]'; // crown + gold glow
        if (streak >= 14) return 'text-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.6)]'; // gold glow
        if (streak >= 7) return 'text-red-500'; // bright red
        if (streak >= 4) return 'text-red-400'; // red
        return 'text-orange-500'; // orange
    };
  
    const renderBlazeStreak = (streak: number) => {
        if (streak === 0) return null;
        return (
            <div className="flex items-center gap-0.5 group relative cursor-pointer ml-1 text-sm font-bold tracking-tight">
                {streak >= 30 && <span className="text-[10px] -mr-0.5 mb-2 absolute -top-2 -left-1">👑</span>}
                <span className={`text-[12px] ${getStreakColor(streak)}`}>🔥{streak}</span>
                <div className="absolute hidden group-hover:block bottom-full mb-2 left-1/2 -translate-x-1/2 w-32 bg-black/90 text-white text-[10px] p-2 rounded-lg border border-white/10 z-50 shadow-xl pointer-events-none text-center">
                    {streak} day streak! Message today to keep it alive 🔥
                </div>
            </div>
        );
    };
            ...
            >
                ...
                <div className="flex-1 min-w-0 pr-2 pb-0.5 pt-0.5 flex flex-col justify-between h-[44px]">
                    <div className="flex items-center">
                        ...
                        {renderBlazeStreak(chat.blazeStreak)}
...
export default function ConnectScreen() {
  ...
  return (
    <div className="w-full h-full flex flex-col pt-4 pb-24 relative overflow-hidden bg-[#0A0A12] text-white">
          ...
          {filteredChats.length === 0 ? (
              ...
              <div className="flex flex-col">
                  {filteredChats.map(chat => (
                     <SwipeableChatRow key={chat.id} chat={chat} onClick={() => navigate(`/connect?user=${chat.id}`)} />
                  ))}
              </div>
          )}
      </div>
      ...
      {/* FAB - Compose */}
      <button className="absolute bottom-24 right-5 sm:bottom-6 sm:right-6 bg-gradient-to-tr from-[#B026FF] to-[#D869FF] text-white rounded-full px-5 py-3.5 shadow-[0_4px_25px_rgba(176,38,255,0.4)] flex items-center gap-2 hover:scale-105 active:scale-95 transition-all z-40">
