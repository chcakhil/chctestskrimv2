      ...
      {pulseToasts.map(toast => (
        ...
        >
           <div className="bg-[#0a0a0c] rounded-2xl p-3 flex flex-col items-center text-center">
             <p className="text-[#00F0FF] font-bold text-lg mb-0.5">⚡ +{toast.points} Pulse Points!</p>
             <p className="text-white/90 text-sm mb-1">{toast.message.replace(/\+[0-9]+ Pulse.*|\+[0-9]+ ⚡.*/, '').trim()}</p>
